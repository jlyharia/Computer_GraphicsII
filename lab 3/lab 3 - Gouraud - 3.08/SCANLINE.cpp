/** Tasks in SCANLINE.cpp
    ...
    in the meantime, implement Phong illumination model and Gouraud shading
**/
#include "SCANLINE.h"
#include "TABLE.h"
#include <vector>
        ...
        else //exist edge in Edge Table
        {
            ...
            {//compute Z value and Intensity
                ...
                else
                {
                    ...
                    //I = I + (I2 - I2)/ (X2 - X1)
                    //linear interpolate intensity on X
                    R_increOnX = (AET[j+1][5] - AET[j][5])/
                            (ceil(AET[j+1][1]) - ceil(AET[j][1]));
                    G_increOnX = (AET[j+1][7] - AET[j][7])/
                            (ceil(AET[j+1][1]) - ceil(AET[j][1]));
                    B_increOnX = (AET[j+1][9] - AET[j][9])/
                            (ceil(AET[j+1][1]) - ceil(AET[j][1]));
                }
                ...
                {//fill pixel
                    ...
                    else
                    {
                        ...
                        temp_pixel[y_min][X_value][2] =
                            temp_pixel[y_min][X_value -1 ][2] + R_increOnX;
                        temp_pixel[y_min][X_value][3] =
                            temp_pixel[y_min][X_value -1 ][3] + G_increOnX;
                        temp_pixel[y_min][X_value][4] =
                            temp_pixel[y_min][X_value -1 ][4] + B_increOnX;
                    }
                    ///--------------Z buffer---------------------------------
                    if(real_pixel[y_min][X_value][0] == 0)
                    {//if there doesn't exit pixel
                        real_pixel[y_min][X_value][0]= 1;
                        for(int i = 1 ; i < 5 ; i++)//replace pixel value to real buffer
            ...
            {//increase X value
                ...
                //increase RBG value on edge
                AET[j][5] = AET[j][5] + AET[j][6];//R
                AET[j][7] = AET[j][7] + AET[j][8];//G
                AET[j][9] = AET[j][9] + AET[j][10];//B
            }
        }
...
//                X_incre = 0;
            ...
            {//(z1 - z2) / (y1 - y2)
                ...
                // RGB increment (R1 - R2) / (y1 - y2)
                R_incre = (nearVertex[i*2 + j][5] - startVertex[i][5])/
                            (nearVertex[i*2 + j][1] - startVertex[i][1]);
                G_incre = (nearVertex[i*2 + j][6] - startVertex[i][6])/
                            (nearVertex[i*2 + j][1] - startVertex[i][1]);
                B_incre = (nearVertex[i*2 + j][7] - startVertex[i][7])/
                        (nearVertex[i*2 + j][1] - startVertex[i][1]);
            }
            ...
            (edgeValue[0] , startVertex[i][0] , X_incre,
             ...
             startVertex[i][5],R_incre,
             startVertex[i][6],G_incre,
             startVertex[i][7],B_incre);
                ...
                int poly_num)
{
    for (int i = 0; i < (int)Ycompare.size() ; i++)
    {
        ...
        //add RGB value to start vertex
        temp_0.insert(temp_0.end(),Light_Intensity[Ycompare[i][0]-1].begin(),
                      Light_Intensity[Ycompare[i][0]-1].end());
        ...
        temp_1.insert(temp_1.end(),Light_Intensity[Ycompare[i][2]-1].begin(),
                      Light_Intensity[Ycompare[i][2]-1].end());

        temp_2.insert(temp_2.end(),Light_Intensity[Ycompare[i][3]-1].begin(),
                      Light_Intensity[Ycompare[i][3]-1].end());
