...
{   //compute back face and normalized vertices data
    ...
    ///-----Compose M_view and M_perspective for invers Matrix----
    matrix_ViewAndPers();
    ///-------------------------------------------------
    ...
    for(int i = 0 ; i < (int)vec_world_vertex.size() ; i++)
        matrix_multi_4x1(vec_view_vertex[i] , M_view_4D, vec_world_vertex[i]);
    ...
    {//compute perspective vertices from view space vertices
        ...
        {//ignore back side vertices
            matrix_multi_4x1(vec_temp[i], M_pers_4D, vec_view_vertex[i]);
            //compute vertices in perspective space without back side vertices
            for(int j=0; j<4; j++)
    ...
    ///--------test invert-------------------------
    ///--------test invert-------------------------
//    vector< vector<float> > renormal(vec_device_vertex);
//    //1 renormal
//    for(int i=0;i<(int)renormal.size();i++)
//    {   //scale normalize vertices data to device
//        if(renormal[i].size() == 5)
//        {
//            renormal[i][0] = (2/(Xmax - Xmin))*(vec_device_vertex[i][0])-1 ;//x
//            renormal[i][1] = (2/(Ymax - Ymin))*(vec_device_vertex[i][1])-1 ;//y
//        }
//    }
//
//    //2 times invert matrix
//    vector< vector<float> > vec_invert(renormal);
//    for(int i=0;i<(int)renormal.size();i++)
//    {   //scale normalize vertices data to device
//        matrix_multi_4x1(vec_invert[i], M_Invert, renormal[i]);
//    }
//    for(int i = 0 ; i < (int)vec_invert.size() ; i++)
//    {
//        datatest<<i<<'\t';
//        for(int j = 0; j<4; j++)
//            datatest<<setw(15)<<vec_invert[i][j]/vec_invert[i][3];
//        datatest<<endl;
//    }
...
void loadModel::setShiftLocal(float *offset)
{   //set vector that object shift from local to world
    offset_world.push_back(offset[0]);
    offset_world.push_back(offset[1]);
    offset_world.push_back(offset[2]);
}
...
void loadModel::matrix_ViewAndPers()
{//Compose M_view and M_perspective for invers Matrix
    vector< vector<float> > M_ViewAndPers_4D(4, vector<float> (4));
    vector< vector<float> > temp_M_Invert(4, vector<float> (4));
    matrix_multi_4x4(M_ViewAndPers_4D,M_pers_4D,M_view_4D);

    invert(M_ViewAndPers_4D,temp_M_Invert);
//    invert(M_pers_4D,temp_M_Invert);
    M_Invert = temp_M_Invert;

}
float loadModel::determinant(vector< vector<float> > in_Matrix)
{
    float value;
    value =
        in_Matrix[0][3]*in_Matrix[1][2]*in_Matrix[2][1]*in_Matrix[3][0] -
        in_Matrix[0][2]*in_Matrix[1][3]*in_Matrix[2][1]*in_Matrix[3][0] -
        in_Matrix[0][3]*in_Matrix[1][1]*in_Matrix[2][2]*in_Matrix[3][0] +
        in_Matrix[0][1]*in_Matrix[1][3]*in_Matrix[2][2]*in_Matrix[3][0] +
        in_Matrix[0][2]*in_Matrix[1][1]*in_Matrix[2][3]*in_Matrix[3][0] -
        in_Matrix[0][1]*in_Matrix[1][2]*in_Matrix[2][3]*in_Matrix[3][0] -
        in_Matrix[0][3]*in_Matrix[1][2]*in_Matrix[2][0]*in_Matrix[3][1] +
        in_Matrix[0][2]*in_Matrix[1][3]*in_Matrix[2][0]*in_Matrix[3][1] +
        in_Matrix[0][3]*in_Matrix[1][0]*in_Matrix[2][2]*in_Matrix[3][1] -
        in_Matrix[0][0]*in_Matrix[1][3]*in_Matrix[2][2]*in_Matrix[3][1] -
        in_Matrix[0][2]*in_Matrix[1][0]*in_Matrix[2][3]*in_Matrix[3][1] +
        in_Matrix[0][0]*in_Matrix[1][2]*in_Matrix[2][3]*in_Matrix[3][1] +
        in_Matrix[0][3]*in_Matrix[1][1]*in_Matrix[2][0]*in_Matrix[3][2] -
        in_Matrix[0][1]*in_Matrix[1][3]*in_Matrix[2][0]*in_Matrix[3][2] -
        in_Matrix[0][3]*in_Matrix[1][0]*in_Matrix[2][1]*in_Matrix[3][2] +
        in_Matrix[0][0]*in_Matrix[1][3]*in_Matrix[2][1]*in_Matrix[3][2] +
        in_Matrix[0][1]*in_Matrix[1][0]*in_Matrix[2][3]*in_Matrix[3][2] -
        in_Matrix[0][0]*in_Matrix[1][1]*in_Matrix[2][3]*in_Matrix[3][2] -
        in_Matrix[0][2]*in_Matrix[1][1]*in_Matrix[2][0]*in_Matrix[3][3] +
        in_Matrix[0][1]*in_Matrix[1][2]*in_Matrix[2][0]*in_Matrix[3][3] +
        in_Matrix[0][2]*in_Matrix[1][0]*in_Matrix[2][1]*in_Matrix[3][3] -
        in_Matrix[0][0]*in_Matrix[1][2]*in_Matrix[2][1]*in_Matrix[3][3] -
        in_Matrix[0][1]*in_Matrix[1][0]*in_Matrix[2][2]*in_Matrix[3][3] +
        in_Matrix[0][0]*in_Matrix[1][1]*in_Matrix[2][2]*in_Matrix[3][3];
    return value;
}
void loadModel::invert(vector< vector<float> > in_Matrix,
                vector< vector<float> > &out_Matrix)
{
    float det = determinant(in_Matrix);
    det = 1/det;
    out_Matrix[0][0] = in_Matrix[1][2]*in_Matrix[2][3]*in_Matrix[3][1] -
                        in_Matrix[1][3]*in_Matrix[2][2]*in_Matrix[3][1] +
                        in_Matrix[1][3]*in_Matrix[2][1]*in_Matrix[3][2] -
                        in_Matrix[1][1]*in_Matrix[2][3]*in_Matrix[3][2] -
                        in_Matrix[1][2]*in_Matrix[2][1]*in_Matrix[3][3] +
                        in_Matrix[1][1]*in_Matrix[2][2]*in_Matrix[3][3];
    out_Matrix[0][1] = in_Matrix[0][3]*in_Matrix[2][2]*in_Matrix[3][1] -
                        in_Matrix[0][2]*in_Matrix[2][3]*in_Matrix[3][1] -
                        in_Matrix[0][3]*in_Matrix[2][1]*in_Matrix[3][2] +
                        in_Matrix[0][1]*in_Matrix[2][3]*in_Matrix[3][2] +
                        in_Matrix[0][2]*in_Matrix[2][1]*in_Matrix[3][3] -
                        in_Matrix[0][1]*in_Matrix[2][2]*in_Matrix[3][3];
    out_Matrix[0][2] = in_Matrix[0][2]*in_Matrix[1][3]*in_Matrix[3][1] -
                        in_Matrix[0][3]*in_Matrix[1][2]*in_Matrix[3][1] +
                        in_Matrix[0][3]*in_Matrix[1][1]*in_Matrix[3][2] -
                        in_Matrix[0][1]*in_Matrix[1][3]*in_Matrix[3][2] -
                        in_Matrix[0][2]*in_Matrix[1][1]*in_Matrix[3][3] +
                        in_Matrix[0][1]*in_Matrix[1][2]*in_Matrix[3][3];
    out_Matrix[0][3] = in_Matrix[0][3]*in_Matrix[1][2]*in_Matrix[2][1] -
                        in_Matrix[0][2]*in_Matrix[1][3]*in_Matrix[2][1] -
                        in_Matrix[0][3]*in_Matrix[1][1]*in_Matrix[2][2] +
                        in_Matrix[0][1]*in_Matrix[1][3]*in_Matrix[2][2] +
                        in_Matrix[0][2]*in_Matrix[1][1]*in_Matrix[2][3] -
                        in_Matrix[0][1]*in_Matrix[1][2]*in_Matrix[2][3];
    out_Matrix[1][0] = in_Matrix[1][3]*in_Matrix[2][2]*in_Matrix[3][0] -
                        in_Matrix[1][2]*in_Matrix[2][3]*in_Matrix[3][0] -
                        in_Matrix[1][3]*in_Matrix[2][0]*in_Matrix[3][2] +
                        in_Matrix[1][0]*in_Matrix[2][3]*in_Matrix[3][2] +
                        in_Matrix[1][2]*in_Matrix[2][0]*in_Matrix[3][3] -
                        in_Matrix[1][0]*in_Matrix[2][2]*in_Matrix[3][3];
    out_Matrix[1][1] = in_Matrix[0][2]*in_Matrix[2][3]*in_Matrix[3][0] -
                        in_Matrix[0][3]*in_Matrix[2][2]*in_Matrix[3][0] +
                        in_Matrix[0][3]*in_Matrix[2][0]*in_Matrix[3][2] -
                        in_Matrix[0][0]*in_Matrix[2][3]*in_Matrix[3][2] -
                        in_Matrix[0][2]*in_Matrix[2][0]*in_Matrix[3][3] +
                        in_Matrix[0][0]*in_Matrix[2][2]*in_Matrix[3][3];
    out_Matrix[1][2] = in_Matrix[0][3]*in_Matrix[1][2]*in_Matrix[3][0] -
                        in_Matrix[0][2]*in_Matrix[1][3]*in_Matrix[3][0] -
                        in_Matrix[0][3]*in_Matrix[1][0]*in_Matrix[3][2] +
                        in_Matrix[0][0]*in_Matrix[1][3]*in_Matrix[3][2] +
                        in_Matrix[0][2]*in_Matrix[1][0]*in_Matrix[3][3] -
                        in_Matrix[0][0]*in_Matrix[1][2]*in_Matrix[3][3];
    out_Matrix[1][3] = in_Matrix[0][2]*in_Matrix[1][3]*in_Matrix[2][0] -
                        in_Matrix[0][3]*in_Matrix[1][2]*in_Matrix[2][0] +
                        in_Matrix[0][3]*in_Matrix[1][0]*in_Matrix[2][2] -
                        in_Matrix[0][0]*in_Matrix[1][3]*in_Matrix[2][2] -
                        in_Matrix[0][2]*in_Matrix[1][0]*in_Matrix[2][3] +
                        in_Matrix[0][0]*in_Matrix[1][2]*in_Matrix[2][3];
    out_Matrix[2][0] = in_Matrix[1][1]*in_Matrix[2][3]*in_Matrix[3][0] -
                        in_Matrix[1][3]*in_Matrix[2][1]*in_Matrix[3][0] +
                        in_Matrix[1][3]*in_Matrix[2][0]*in_Matrix[3][1] -
                        in_Matrix[1][0]*in_Matrix[2][3]*in_Matrix[3][1] -
                        in_Matrix[1][1]*in_Matrix[2][0]*in_Matrix[3][3] +
                        in_Matrix[1][0]*in_Matrix[2][1]*in_Matrix[3][3];
    out_Matrix[2][1] = in_Matrix[0][3]*in_Matrix[2][1]*in_Matrix[3][0] -
                        in_Matrix[0][1]*in_Matrix[2][3]*in_Matrix[3][0] -
                        in_Matrix[0][3]*in_Matrix[2][0]*in_Matrix[3][1] +
                        in_Matrix[0][0]*in_Matrix[2][3]*in_Matrix[3][1] +
                        in_Matrix[0][1]*in_Matrix[2][0]*in_Matrix[3][3] -
                        in_Matrix[0][0]*in_Matrix[2][1]*in_Matrix[3][3];
    out_Matrix[2][2] = in_Matrix[0][1]*in_Matrix[1][3]*in_Matrix[3][0] -
                        in_Matrix[0][3]*in_Matrix[1][1]*in_Matrix[3][0] +
                        in_Matrix[0][3]*in_Matrix[1][0]*in_Matrix[3][1] -
                        in_Matrix[0][0]*in_Matrix[1][3]*in_Matrix[3][1] -
                        in_Matrix[0][1]*in_Matrix[1][0]*in_Matrix[3][3] +
                        in_Matrix[0][0]*in_Matrix[1][1]*in_Matrix[3][3];
    out_Matrix[2][3] = in_Matrix[0][3]*in_Matrix[1][1]*in_Matrix[2][0] -
                        in_Matrix[0][1]*in_Matrix[1][3]*in_Matrix[2][0] -
                        in_Matrix[0][3]*in_Matrix[1][0]*in_Matrix[2][1] +
                        in_Matrix[0][0]*in_Matrix[1][3]*in_Matrix[2][1] +
                        in_Matrix[0][1]*in_Matrix[1][0]*in_Matrix[2][3] -
                        in_Matrix[0][0]*in_Matrix[1][1]*in_Matrix[2][3];
    out_Matrix[3][0] = in_Matrix[1][2]*in_Matrix[2][1]*in_Matrix[3][0] -
                        in_Matrix[1][1]*in_Matrix[2][2]*in_Matrix[3][0] -
                        in_Matrix[1][2]*in_Matrix[2][0]*in_Matrix[3][1] +
                        in_Matrix[1][0]*in_Matrix[2][2]*in_Matrix[3][1] +
                        in_Matrix[1][1]*in_Matrix[2][0]*in_Matrix[3][2] -
                        in_Matrix[1][0]*in_Matrix[2][1]*in_Matrix[3][2];
    out_Matrix[3][1] = in_Matrix[0][1]*in_Matrix[2][2]*in_Matrix[3][0] -
                        in_Matrix[0][2]*in_Matrix[2][1]*in_Matrix[3][0] +
                        in_Matrix[0][2]*in_Matrix[2][0]*in_Matrix[3][1] -
                        in_Matrix[0][0]*in_Matrix[2][2]*in_Matrix[3][1] -
                        in_Matrix[0][1]*in_Matrix[2][0]*in_Matrix[3][2] +
                        in_Matrix[0][0]*in_Matrix[2][1]*in_Matrix[3][2];
    out_Matrix[3][2] = in_Matrix[0][2]*in_Matrix[1][1]*in_Matrix[3][0] -
                        in_Matrix[0][1]*in_Matrix[1][2]*in_Matrix[3][0] -
                        in_Matrix[0][2]*in_Matrix[1][0]*in_Matrix[3][1] +
                        in_Matrix[0][0]*in_Matrix[1][2]*in_Matrix[3][1] +
                        in_Matrix[0][1]*in_Matrix[1][0]*in_Matrix[3][2] -
                        in_Matrix[0][0]*in_Matrix[1][1]*in_Matrix[3][2];
    out_Matrix[3][3] = in_Matrix[0][1]*in_Matrix[1][2]*in_Matrix[2][0] -
                        in_Matrix[0][2]*in_Matrix[1][1]*in_Matrix[2][0] +
                        in_Matrix[0][2]*in_Matrix[1][0]*in_Matrix[2][1] -
                        in_Matrix[0][0]*in_Matrix[1][2]*in_Matrix[2][1] -
                        in_Matrix[0][1]*in_Matrix[1][0]*in_Matrix[2][2] +
                        in_Matrix[0][0]*in_Matrix[1][1]*in_Matrix[2][2];
    for (int i=0;i<4 ;i++)
        for (int j=0;j<4 ;j++)
            out_Matrix[i][j] = out_Matrix[i][j]*det;
