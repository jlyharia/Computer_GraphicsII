...
void SCANLINE::Compute_Poly_Normal()
{
    ...
    //light intensity
    I_light.push_back(0.5); //Red
    I_light.push_back(0.5); //Green
    I_light.push_back(0.5); //Blue
...
//
//        }
        Normalization(temp_vertex_normal[i]);
    }
...
//    float color[3]={0.4, 0.4, 0.4};
    ...
    for (int i = 0; i< Ymax ; i++ )
    	for (int j = 0; j< Xmax ; j++ )
    	{
            ...
            float final_noise = InterpolateCylinder(cylinder_u,cylinder_v);
            ///-------------------------------
            ...
            for (int p = 0; p< 3 ; p++ )
            {
                ...
                real_pixel[i][j][p+2] = Intensity_diffuse[p] +
                    ...
                    +final_noise*color[p];
//                real_pixel[i][j][p+2] = final_noise*color[p];
    ...
    for(int y_min = 0; y_min< (int)entry.size(); y_min++) //i -> Y min
    {
        ...
        else //exist edge in Edge Table
        {
            ...
            {//compute Z value and RGB value
                ...
                {//fill pixel
                    vector< vector<float> > scan_pixel_normal
                            (ceil(AET[j+1][1]) +1 ,vector<float>(3));
...
void SCANLINE::compute_noise()
{//compute noise, change noise function need to change here and PerlinNoiseTexutre.cpp
    texture_width = texture_height = 512;
    vector< vector<float> > temp_Texel_noise(texture_width,
                                             vector<float> (texture_height));
    for (int i= 0 ; i< texture_width; i++)
        for (int j=0; j< texture_height ; j++)
        {

            temp_Texel_noise[i][j] = (PerlinNoise_2D(i,j) +1)/2;
            //Sum 1/ f (noise)
//            temp_Texel_noise[i][j] = PerlinNoise_2D(i,j);
            //Sin (x+ sum 1/f(|noise|)) and  Sum 1/ f (|noise|)

        }
    Texel_noise = temp_Texel_noise;

}
float SCANLINE::InterpolateCylinder(float x, float y)
{
    int integer_X = (int)x;
    ...
    float v1, v2, v3, v4;
    v1 = Texel_noise[integer_X][integer_Y];
    v2 = Texel_noise[integer_X + 1][integer_Y];
    v3 = Texel_noise[integer_X][integer_Y + 1];
    v4 = Texel_noise[integer_X + 1][integer_Y + 1];
